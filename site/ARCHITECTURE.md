...
**Programs Manager** é uma aplicação React + TypeScript que monitora logs em tempo real de um servidor local (porta 8000), com interface moderna usando Tailwind CSS e Radix UI.
...
┌─────────────────────────────────────────────────────────────┐
│            Backend / Log Server (localhost:8000)            │
│  ┌──────────────────────────────────────────────────────┐   │
...
export function classifyLogSection(timestamp: Date, siteLoadedAt: Date): "current" | "history"
...
```ts
// .env.local
VITE_LOG_SERVER_URL=http://localhost:8000/log.log
...
|-------|-------|----------|
| "Cannot GET /log.log" | Server not running | Start: `python -m http.server 8000` |
| "Porta 8000 indisponível" | Timeout after 30s | Check server, increase timeout |
